...
def setWord2VecModelConfiguration(v2i,w2v,d_w2v,d_lproj,file_type):
	'''
	...
	if file_type == 'MAK' or file_type == 'GOOGLE':
	...
	elif file_type == 'OURS' or file_type == 'OURS_TVQA' or file_type == 'OURS_TEST' or file_type == 'SUBTITLES_100' or file_type == 'SUBTITLES_300':
		for w, v in v2i.iteritems():
			# pdb.set_trace()
			# if w in w2v.vocab:
			if w in w2v.wv.vocab:
	...
	# np_mask and T_mask are len(v2i) x 300 matrix with 1st row all zero and all other rows 1...they are constants
	# LUT and T_w2v are len(v2i) x 300 matrix and 1st row is made all zeros and rest rows are taken from the wor2vec...they are constants...so T_w2v is actual w2v
	# pca_mat is None for he case that d_w2v == d_lproj
	# T_B is a variable and it's a 300 x 300 matrix intlized as an identity matrix for the case that d_w2v == d_lproj

	return T_B, T_w2v, T_mask, pca_mat


def setWord2VecModelConfiguration_old(v2i,w2v,d_w2v,d_lproj):
	'''
		v2i: vocab(word) to int(index)
		w2v: word to vector
		d_w2v:dimension of w2v
...
def setWord2VecModelConfiguration_combine_train_val_BJ(v2i,w2v_train,w2v_val,d_w2v,d_lproj):
	'''
v	
		v2i: vocab(word) to int(index)
		w2v: word to vector
	...
	found_words_train = 0
	found_words_val = 0
	found_words_common = 0

	for w, v in v2i.iteritems():

		if w in w2v_train.wv.vocab and w in w2v_val.wv.vocab:
			found_words_common +=1
			found_words +=1
			LUT[v] = (w2v_val[w] + w2v_train[w])/2.0 
		elif w in w2v_val.wv.vocab:
			# pdb.set_trace()
			# LUT[v] = w2v_val.get_vector(w)
			LUT[v] = w2v_val[w]
			found_words +=1
			found_words_val +=1
		elif w in w2v_train.wv.vocab:
			# pdb.set_trace()
			# LUT[v] = w2v_train.get_vector(w)
			LUT[v] = w2v_train[w]
			found_words +=1
			found_words_train +=1
		else:
	...
	print "Found          %d / %d words" %(found_words, len(v2i))
	print "Found train    %d / %d words" %(found_words_train, len(v2i))
	print "Found val      %d / %d words" %(found_words_val, len(v2i))
	print "Found common   %d / %d words" %(found_words_common, len(v2i))
...
def setWord2VecModelConfiguration_custom_BJ(v2i,w2v_g,w2v_m,d_w2v,d_lproj):
	'''
		v2i: vocab(word) to int(index)
		w2v: word to vector
		d_w2v:dimension of w2v
	...
	for w, v in v2i.iteritems():
		if w in w2v_g.vocab:
			LUT[v] = w2v_g.get_vector(w)
			found_words +=1
		elif w in w2v_m.vocab:
			LUT[v] = w2v_m.get_vector(w)
			found_words +=1			
		else:
			print(w)
			LUT[v] = rng.randn(d_w2v)
...
def getEmbeddingWithWord2Vec(words, T_w2v, T_mask):
	...
	# tf.gather(T_w2v,words) = tf.Tensor 'video_subtitle_hierarchical_frame_clip/Gather_5:0' shape=(?, 25, 300) dtype=float32
	# tf.gather(T_mask,words) ...should be ones of 300d except for 0th row it will be all zeros

	# words = tf.Tensor 'input_question:0' shape=(?, 25) dtype=int32
	# input_shape = [None, 25]
	# mask = tf.Tensor 'video_subtitle_hierarchical_frame_clip/NotEqual:0' shape=(?, 25) dtype=bool
	# embeded_words = tf.Tensor 'video_subtitle_hierarchical_frame_clip/mul:0' shape=(?, 25, 300) dtype=float32

	return embeded_words, mask 

def getEmbeddingWithWord2Vec_custom_BJ(words, T_w2v, T_mask, T_w2v_val, train_mode):
	...
	# tf.print(train_mode)
	# print(tf.constant(np.ones((1)).astype('float32')))
	# if tf.equal(train_mode,tf.constant(np.ones((1)).astype('float32'))):



	embeded_words = tf.cond(train_mode, lambda: tf.gather(T_w2v,words)*tf.gather(T_mask,words) , lambda: tf.gather(T_w2v_val,words)*tf.gather(T_mask,words))

	# if tf.cond(train_mode,lambda: 1,lambda: 0):
	# 	embeded_words = tf.gather(T_w2v,words)*tf.gather(T_mask,words)
	# else: 
	# 	embeded_words = tf.gather(T_w2v_val,words)*tf.gather(T_mask,words)
...
def getClassifierLoss_QA_only(T_q, T_a, answer_index=None, isTest=False):
...
def getVideoSemanticEmbedding(x,w2v,T_B,pca_mat=None):   ### Input is 32x7x7x512, outputs is 1x300 
	'''
	...
	input_shape = x.get_shape().as_list()  #[None, 32, 512, 7, 7] 
	w2v_shape = w2v.get_shape().as_list()  #[26033, 300]
	assert(len(input_shape)==5)
	axis = [0,1,3,4,2]
	x = tf.transpose(x,perm=axis)			# shape=(?, 32, 7, 7, 512)
	x = tf.reshape(x,(-1,input_shape[2]))   # shape=(?, 512).....linearizing the visual features.....input_shape[2] = 512	
	# x = tf.nn.l2_normalize(x,-1)

	if pca_mat is not None:
		linear_proj = tf.Variable(0.1*pca_mat,dtype='float32',name='visual_linear_proj') #shape=(512, 300).....so this converts the visual 512d feature to 300d
	else:
		linear_proj = init_weight_variable((input_shape[2],w2v_shape[-1]), init_method='uniform', name='visual_linear_proj')

	x = tf.matmul(x,linear_proj)   # shape=(?, 300)
	x = tf.nn.l2_normalize(x,-1)

	w2v_cov = tf.matmul(tf.transpose(w2v,perm=[1,0]),w2v) #w2v_cov --> 300x26033 x 26033x300 --> 300x300....this kind of dot product of the matrix with itself

	# pdb.set_trace()
	x = tf.matmul(x,w2v_cov) # (batch_size*timesteps*height*width, |V|)    ....shape=(?, 300)  this is like x  = x* w2v' * w2v

	x = tf.reshape(x,(-1,input_shape[1],input_shape[3],input_shape[4],w2v_shape[-1]))   # shape=(?, 32, 7, 7, 300)
	axis = [0,1,4,2,3]
	x = tf.transpose(x,perm=axis)		# shape=(?, 32, 300, 7, 7)
	
	# can be extended to different architecture
	x = tf.reduce_sum(x,reduction_indices=[1,3,4])    # shape=(?, 300)....this is like summing the 32,7,7 into 1 
	x = tf.nn.l2_normalize(x,-1)

	x = tf.matmul(x,T_B)   # T_B --> shape=(300, 300),....finally x --> shape=(?,300)

	

	return x # shape=(?,300)...the ? is = batch_size

#### QQ Why do they do X x w2v' x w2v to convert the 512 to 300 dim and then sum across 32x7x7 into a single no.
